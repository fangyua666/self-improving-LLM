...
from src.data import generate_origin_dataset
...
from src.self_improvement import run_self_improvement_mv, run_self_improvement_lenth_or_no

def parse_args():
    parser = argparse.ArgumentParser(description="Train and perform self-improvement on a GPT model")
    ...
    parser.add_argument("--block_size", type=int, default=100, help="Maximum sequence length")
    ...
    parser.add_argument("--decay", type=int, default=1500, help="Decay steps for scheduler") # change for string copy, 500
...
def main():
    ...
    vocab_size = 14 # 14 for addition, 13 for string copy
    task = 'reverse_addition'  
    if task == 'reverse_addition':
        task_simplified = 'ra'
    else:
        task_simplified = 'sc'
    ...
    if args.train_multiple_base:
        ...
        train_multiple_base_models(
            ...
            data_path=os.path.join(args.data_dir, f"origin_ds_{task}.txt"),
    ...
    if not args.skip_base_model_train:
        ...
        model = train_base_model(
            ...
            eval_interval=args.eval_interval,
            data_path=os.path.join(args.data_dir, f"origin_ds_{task}.txt"),
            save_path=os.path.join(args.models_dir, f"{task_simplified}_model_0.pt"),
            device=args.device,
            task=task
        )
        base_model_path = os.path.join(args.models_dir, f"{task_simplified}_model_0.pt")
    else:
    ...
    if not args.skip_si:
        ...
        if args.si_method == "length_filter" or args.si_method == "no_filter":
            si_function = run_self_improvement_lenth_or_no
        ...
        si_function(
            ...
            vocab_size=vocab_size,
            task=task,
            filter="length" if args.si_method == "length_filter" else "no_filter",
        )
